...
2. The R markdown (Rmd) file for this tutorial. [Link](data/de_tutorial_exampleworkflow_2025.07.22.Rmd).
3. The sample sheet that relates sample IDs to experimental conditions. [Link](data/dme_elev_samples.tsv).
4. The gene-level count matrix generated by the *nf-core/rnaseq* workflow. [Link](data/salmon.merged.gene_counts.tsv).
...
!!! warning "We do not recommend using single-end data for expression analysis"
    ...
    See: Freedman et al. 2020, BMC Bioinformatics [https://link.springer.com/article/10.1186/s12859-020-3484-z](https://link.springer.com/article/10.1186/s12859-020-3484-z)
...
This tutorial performs differential expression analysis with R, and for the purposes of making debugging and visualization/plotting easier, we recommend using [RStudio], an interactive environment that can downloaded from [here](https://posit.co/download/rstudio-desktop/). 
...
### 6. TMM normalization
The next step is conducting normalization of the counts across samples so that library size differences and the effects of genes that are highly expressed and sample specific are accounted for. Regarding the latter, we want to avoid having a few genes take up expression sequencing "real estate" given the overall number of reads generated by a sample, such that it reduces the reads in other transcripts in a way that would lead to false positive DE. To do this, we use the trimmed mean of M-values (TMM)  of [Robinson and Oshlack (2010)](https://doi.org/10.1186/gb-2010-11-3-r25) available in edgeR. Note, one can use other normalization schemes, and I have seen some evidence that conditional quantile normalizartion (CQN) mignt be worth considering as an alternative.
