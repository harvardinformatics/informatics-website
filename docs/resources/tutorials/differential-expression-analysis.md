...
If you want to reproduce the workflow explained below, the R markdown (Rmd) file for it can be found [here](data/de_tutorial_exampleworkflow_2025.07.22.Rmd). The sample sheet that relates sample IDs to experimental conditions can be found at [sample_sheet](data/data/dme_elev_samples.tsv), and the gene-level count matrix can be found at [count_matrix](data/salmon.merged.gene_counts.tsv).
