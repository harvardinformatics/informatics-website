...
## Re-run the make-beds.sh script
...
"I want to loop over the an arbitrary list of bed files and run the snp-density.sh script on each one, and then print out the results in a file called snp-densities.txt. The script should be run like bash script-name.sh list-of-bed-files.txt vcf-file.vcf > snp-densities.txt"
...
> Then let's run the script in the terminal and redirect the output to file called `snp-densities.txt`:

```bash

## Run snp-desnity-5.sh after you have created it
...
We've come a long way since the our first one-liner script `snp-density.sh`! Let's copy `snp-density-5.sh` to a new version called `snp-density-final.sh` where we'll put on the finishing touches that make the script easier to use, read, and monitor. 
...
> **Exercise**: Add a description of what the script does, what the inputs are, what the outputs are, and the author of the script to the top of your `snp-density-final.sh` script.
