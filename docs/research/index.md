...
For several years prior to our study, the standard, cost-effective approach for estimating gene expression estimates with RNA-seq data and a reference genome, was to sequence moderate-length single end reads (typically 75 base pairs long). Our intuition was that,for essentially the same cost, one could obtain higher specificity for read alignment with short paired-end reads, and this would lead to more robust expression estimates and downstream tests of differential expression. This research project confirmed that our intuition was correct.
